...
function migrate() {
    echo -e "\n ... migrate db ... \n"
    docker exec -t $1 ./manage.py migrate
}
...
function create_admin_user() {
    docker exec -it ${API_CONTAINER_NAME} ./manage.py shell -c "from django.contrib.auth import get_user_model; User = get_user_model(); User.objects.create_superuser('${1:-admin1}', '', '${2:-test1234}')"
}
